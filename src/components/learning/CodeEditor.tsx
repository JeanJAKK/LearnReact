...
export default function CodeEditor({ 
  ...
  const [testResults, setTestResults] = useState<Array<Test & { passed: boolean }>>([]);
  ...
  const handleRun = useCallback(() => {
    ...
    try {
      ...
      const balancedBrackets = (str: string) => {
        const stack: string[] = [];
    ...
    <div className="rounded-xl overflow-hidden border border-border bg-card shadow-lg">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-muted/50 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-muted-foreground ml-3 font-mono">{language.toUpperCase()}</span>
        </div>
        <div className="flex items-center gap-2">
          <Button 
            variant="ghost" 
            ...
            {testResults.map((test, index) => (
              <div 
                ...
                className={cn(
                  ...
                  test.passed ? "text-success bg-success/10" : "text-destructive bg-destructive/10"
                )}
              >
                {test.passed ? (
      ...
      {(output || error) && (
        <div className={cn(
          ...
          error ? "bg-destructive/10 text-destructive border-destructive/20" : "bg-muted/30 text-foreground"
        )}>
